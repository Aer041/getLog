...
int main(int argc, char** argv)
{
    ...
    if (connection_result != ConnectionResult::Success) {
        std::cerr << "Connection telemetry failed: " << connection_result << '\n';
        return 1;
    }
    ...
    auto log_files = std::make_shared<LogFiles>(system_usb);
    ...
    telemetry.subscribe_position([&](Telemetry::Position position) {
        //std::cout << "subscribe position working \n";
        if(_armed)
    ...
    while(true){
        ...
        if(!_armed){
            std::cout << "Not armed, starting log download. \n";
